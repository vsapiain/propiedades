    ...
    this.each(function() {
      ...
      <div class="imageuploadify well">
        ...
        <div class="imageuploadify-images-list text-center">
          ...
          <span class='imageuploadify-message'>Arrastra tus imágenes aquí</span>
          <button type="button" class="btn btn-default">Selecciona una imagen</button>
        </div>
      </div>
      `);
        ...
        if ($(self).closest("form").valid()){
            ...
            for (let index = 0; index < inputs.length; ++index) {
              try{
                  ...
                  else if ($(inputs[index])!= 'undefined' && !inputs[index].getAttribute("type") || ((inputs[index].getAttribute("type").toLowerCase()) !== "checkbox" && (inputs[index].getAttribute("type").toLowerCase()) !== "radio") || inputs[index].checked) {
            ...
            /*
            if (localStorage.action === undefined && localStorage.action === null ){
                xhr.open("POST", $(this).attr("action"), true);
            }
            else{
                xhr.open("POST", localStorage.action, true);
            ...
            event.stopPropagation();
            event.preventDefault(event);
            var ctls = this.querySelectorAll("select");
            $(ctls).each(function(index){
                var id_select = $(this).attr("id");
                var parent = $(this).parent();
                var error_ = $(this).parent().find("#" + id_select + "-error");
                var error_clone = error_.clone();
                error_.remove();
                if (error_clone.html() !== undefined)
                   parent.append("<label id='" + id_select + "-error" + "' class='error' for='"+ id_select + "'>" + error_clone.html() +"</label>");
            })
